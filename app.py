...
# Stylized title
st.markdown("""
    <div style='text-align: center; font-size: 38px; font-weight: bold; color: #FFFFFF; margin-top: 20px;'>
        🔋 Electric Vehicle Forecasting Tool
    </div>
""", unsafe_allow_html=True)

# Welcome subtitle
st.markdown("""
    <div style='text-align: center; font-size: 20px; font-weight: normal; padding-top: 5px; margin-bottom: 25px; color: #e0e0e0;'>
        Predict and compare EV adoption across counties in Washington State.
    </div>
""", unsafe_allow_html=True)

# Instruction line
st.markdown("""
    <div style='text-align: left; font-size: 18px; padding-top: 10px; color: #e0e0e0;'>
        📍 Select a county to see the 3-year EV adoption forecast.
    </div>
""", unsafe_allow_html=True)
...
ax.set_title(f"EV Adoption - 3 Year Forecast ({county})", fontsize=14, color='white')
...
if historical_total > 0:
    ...
    st.success(f"EV adoption in **{county}** is expected to show a **{trend} of {forecast_growth_pct:.2f}%** over the next 3 years.")
else:
    st.warning("Historical EV total is zero; forecast change cannot be calculated.")

# Compare multiple counties
st.markdown("---")
st.header("🔍 Compare EV Forecasts Across Counties")
...
if multi_counties:
    ...
    st.subheader("📈 County-wise Cumulative EV Forecasts")
    fig, ax = plt.subplots(figsize=(14, 7))
    for cty, group in comp_df.groupby('County'):
        ax.plot(group['Date'], group['Cumulative EV'], marker='o', label=cty)
    ax.set_title("EV Forecast Comparison", fontsize=16, color='white')
    ax.set_xlabel("Date", color='white')
    ax.set_ylabel("Cumulative EV Count", color='white')
    ax.grid(True, alpha=0.3)
    ax.set_facecolor("#1c1c1c")
    ...
    st.success(" | ".join(growth_summaries))

st.markdown("<hr>", unsafe_allow_html=True)
st.markdown("<div style='text-align:center; color:lightgrey;'>Prepared for the <b>AICTE Internship Cycle 2</b> by S4F</div>", unsafe_allow_html=True)
